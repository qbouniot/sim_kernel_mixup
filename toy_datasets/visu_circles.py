...
def show_separation(model, X_train, y_train, X_test, y_test, save=False, name_to_save="", score="", model_name="", font_scale=1.):
    ...
    ax.contourf(xx, yy, probs, 25, alpha=0.8, cmap="RdBu",
                          vmin=0, vmax=1)
    ax_c = f.colorbar(matplotlib.cm.ScalarMappable(cmap="RdBu"), ax=ax, alpha=0.9)
...
show_separation(mlp, X_train, y_train, X_test, y_test, save=save_figs, name_to_save=visu_dir / folder_to_save / "mlp_circles.pdf", score=f"{score:0.3f}", model_name="ERM", font_scale=4.5)
...
show_separation(mlp_mixup, X_train, y_train, X_test, y_test, save=save_figs, name_to_save=visu_dir / folder_to_save / f"mlp_circles_mixup{mixup_alpha}.pdf", model_name="Mixup", score=f"{score_mixup:0.3f}", font_scale=4.5)
...
show_separation(mlp_warp, X_train, y_train, X_test, y_test, save=save_figs, name_to_save= visu_dir / folder_to_save / f"mlp_circles_warp_{tau_max}_{tau_std}.pdf", model_name="Warping Mixup", score=f"{score_warp:0.3f}", font_scale=4.5)
